...
def _resolve_account_url() -> str:
    account_url = os.getenv("TRAVEL_GUIDE_BLOB_ACCOUNT_URL")
    if account_url:
        return account_url.rstrip("/")
    account = os.getenv("TRAVEL_GUIDE_BLOB_ACCOUNT")
    if account:
        return f"https://{account}.blob.core.windows.net"
    raise RuntimeError(
        "Set TRAVEL_GUIDE_BLOB_ACCOUNT_URL (or TRAVEL_GUIDE_BLOB_ACCOUNT) so the travel guide "
        "can be uploaded to Azure Blob Storage."
    )


def upload_to_blob(local_path: Path, blob_name: str, sas_hours: int) -> str:
    ...
    account_url = _resolve_account_url()
    container = os.getenv("TRAVEL_GUIDE_BLOB_CONTAINER", "travel-guides")
    ...
    expiry = now + timedelta(hours=max(sas_hours, 1))
    ...
    return f"{account_url}/{quote(container)}/{quote(blob_name)}?{sas_token}"
...
def main() -> None:
    ...
    filename = f"{slugify(args.city)}-{days}-day-travel-guide.pdf"
    output_path = output_dir / filename

    page_count = build_travel_guide(args.city, days, interests, args.tone, output_path)

    sas_hours = int(os.getenv("TRAVEL_GUIDE_SAS_HOURS", "24"))
    blob_name = f"{datetime.now(timezone.utc).strftime('%Y/%m/%d')}/{filename}"
    share_url = upload_to_blob(output_path, blob_name, sas_hours)
        ...
        json.dumps(
            {
                ...
                "pages": page_count,
                "share_url": share_url,
                "share_url_expires_hours": sas_hours,
                "local_path": display_path(output_path),
                "message": f"Created a colorful PDF travel guide for {args.city} and uploaded it to Azure Blob Storage.",
            },
            indent=2,
        )
    )
